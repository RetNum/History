...
class Experiment(object):
    ...
    def train_and_test(self, dataset_by_split: DatasetBySplit, model, optimizer, scheduler, pbar, num_fold: int)\
        ...
        # 初始化决策历史字典，为每个图维护一个历史
        action_histories = {}
        for epoch in range(self.max_epochs):
            self.train(train_loader=train_loader, model=model, optimizer=optimizer, action_histories=action_histories)
    ...
    def train(self, train_loader, model, optimizer, action_histories):
        model.train()
        ...
        for data in train_loader:
            ...
            # 获取这个图的历史决策信息
            graph_id = data.graph_id.item() if hasattr(data, 'graph_id') else id(data)
            action_history = action_histories.get(graph_id, None)
            ...
            # 更新历史记录
            action_histories[graph_id] = updated_history
    ...
    def test(self, loader, model, split_mask_name: str, calc_edge_ratio: bool,action_histories)\
        ...
        for data in loader:
            ...
            # forward
            scores, edge_ratios,updated_history =\
                model(data.x.to(device=self.device), edge_index=data.edge_index.to(device=self.device),
